...
class Economy(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        self.connection.execute("INSERT OR IGNORE INTO economy (guild_id, user_id, user_name, balance) VALUES (?, ?, ?, ?)", (id, member.id, member.name, STARTING_BALANCE))
        self.connection.commit()
    
    @commands.Cog.listener()
    async def on_member_remove(self, member):
        guild_id = int(member.guild.id)
        self.connection.execute("DELETE FROM economy WHERE guild_id = ? AND user_id = ?", (guild_id, member.id))
        self.connection.commit()
    ...
    def _initDatabase(self):
        ...
        for guild in guilds:
            for member in guild.members:
                self.connection.execute("INSERT OR IGNORE INTO economy (guild_id, user_id, user_name, balance) VALUES (?, ?, ?, ?)", (guild.id, member.id, member.name, STARTING_BALANCE))
        
        self.connection.commit()
    
    def _getBalance(self, guild_id, user_id):
        cursor = self.connection.execute("SELECT balance FROM economy WHERE user_id = ? AND guild_id = ?", (user_id, guild_id))
        balance = cursor.fetchone()[0]
        return balance

    def _setBalance(self, guild_id, user_id, newBalance):
        self.connection.execute("UPDATE economy SET balance = ? WHERE user_id = ? AND guild_id = ?", (newBalance, user_id, guild_id))
        self.connection.commit()
    ...
    async def balance(self, interaction):
        ...
        if balance == None:
            ...
            return
        embed = discord.Embed(title=f"{interaction.user.name}'s Balance", color=discord.Colour.green(), timestamp=datetime.datetime.utcnow())
    ...
    async def pay(self, interaction, user: discord.User, amount: str):
        ...
        self._setBalance(guild_id, user_id, senderBalance - int(amount)) # Subtract from sender
        self._setBalance(guild_id, user.id, receiverBalance + int(amount)) # Add to receiver
    ...
    async def daily(self, interaction):
        ...
        cursor = self.connection.execute("SELECT last_daily FROM economy WHERE user_id = ? AND guild_id = ?", (user_id, guild_id))
        last_daily = cursor.fetchone()[0]
        last_daily = datetime.datetime.strptime(last_daily, "%Y-%m-%d").date() if last_daily != 'NULL' and last_daily is not None else None

        if last_daily == None: # Never Collected Daily Reward
            self.connection.execute("UPDATE economy SET last_daily = ? WHERE user_id = ? AND guild_id = ?", (str(datetime.date.today()), user_id, guild_id))
            self.connection.commit()
    ...
    async def leaderboard(self, interaction):
        guild_id = interaction.guild.id
        cursor = self.connection.execute("SELECT user_name, balance FROM economy WHERE guild_id = ? ORDER BY balance DESC LIMIT 10", (guild_id,))
        embed = discord.Embed(title=f"{interaction.guild.name}'s Leaderboard", color=discord.Colour.green(), timestamp=datetime.datetime.utcnow())
        for row in cursor:
